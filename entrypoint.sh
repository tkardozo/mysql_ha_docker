...
EOF
	...
	if [ ! -z "$MYSQL_INNO_DB_PRIMARY" ]; then
		echo "[Entrypoint] MySQL InnoDB Cluster primary node started"
	fi
        if [ ! -z "$MYSQL_INNO_DB_SECONDARY" ]; then
                echo "[Entrypoint] MySQL InnoDB Cluster secondary node started"
        fi
